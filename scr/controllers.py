...
class GoalStackEntry:
    ...
    def __init__(self, scheme_id, stage_id, state = None):
        ...
        return
    def __repr__(self):
        return "Scheme ID: " + str(self.scheme_id) + " Stage ID: " + str(self.stage_id) + " Saved state:" + str(self.state_save)
...
class ControlScheme:
    ...
    END_STAGE_ID = 'end'
    __cur_stage_id__ = START_STAGE_ID
    is_endless = False
...
class ControllerBase:
    ...
    def set_active_scheme(self, scheme_id, state = None):
        ...
        slot.reset_by_goal_state(scheme.get_cur_stage(), state)
        return True
    ...
    def execute(self):
        scheme = self.get_cur_scheme() #type: ControlScheme
        slot = self.__goal_slot__
    ...
    def advance(self, result):
        ...
        # push scheme
        # end scheme
        scheme = self.__control_schemes__[self.__cur_control_scheme_id__]
        slot = self.__goal_slot__
        if scheme.is_ended():
            self.pop_scheme()
            self.execute()
